...
- **Assistente IA** — Chat especializado em comércio exterior brasileiro (NCM, Incoterms, tributos, documentos, desembaraço, drawback/ex-tarifário), usando a API da Anthropic (Claude Sonnet 4.6) direto do navegador, com contexto automático da transação ativa e aviso permanente de que não substitui despachante, contador ou advogado
...
5. Clique em **Save**
6. Aguarde ~1 minuto e acesse: `https://SEU_USUARIO.github.io/comexapp`
...
1. Baixe a versão vigente em `gov.br/receitafederal` → Legislação → TIPI (formato `.xlsx`)
2. Reprocesse o arquivo com o parser (mapeia colunas NCM/EX/Descrição/Alíquota e gera o JSON de produção)
3. Substitua `data/tipi.json` e confira a data em `data_processamento` e `ultima_atualizacao`
